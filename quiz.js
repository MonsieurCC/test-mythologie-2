...
function renderResults() {
  ...
  app.innerHTML = `
    <div class="card">
      ...
      <h3>Conclusion (placeholder)</h3>
      <p>
        Ici, on affichera le texte de conclusion (≈600 mots) associé au dieu correspondant
        au couple <b>${escapeHtml(top.profile)} → ${escapeHtml(second.profile)}</b>.
      </p>
...
function injectBaseStyles() {
  const css = `
    .card{max-width:820px;margin:24px auto;padding:20px;border:1px solid #ddd;border-radius:12px;font-family:system-ui,Segoe UI,Roboto,Arial}
    .progress{opacity:.75;margin-bottom:8px}
    .options{display:flex;flex-direction:column;gap:10px;margin:16px 0}
    .option{display:flex;gap:10px;align-items:flex-start;padding:10px;border:1px solid #eee;border-radius:10px;cursor:pointer}
    .option:hover{background:#fafafa}
    .optionLabel{min-width:24px;font-weight:700}
    .actions{display:flex;gap:10px;margin-top:12px}
    button{padding:10px 14px;border-radius:10px;border:1px solid #ccc;background:white;cursor:pointer}
    button:disabled{opacity:.5;cursor:not-allowed}
    .hint{margin-top:10px;opacity:.8;font-size:.95em}
    hr{margin:16px 0}
  `;
